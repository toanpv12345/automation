...
int8 c_g,dv_g,c_p,dv_p,c_h,dv_h,thu,c_d,dv_d,c_t,dv_t,c_n,dv_n; 
// khai bao ca bien chua gia tri thap phan cua cac don vi thoi gian
...
void convert_bcd(); 
// chuen doi so 2 bcd thanh 2 so thap phan rieng le bao gom chuc va
//don vi rieng cua cac gia tri thoi gian
...
{  
   ...
   {                            
      ...
      // hien thi dau __ thay vì gia tri de biet minh dang chon gia tri
         //can chinh nao
   }
   else 
   {
      LCD_putchar(c_h + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_h + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   }                           
   printf(LCD_putchar,":");
   if(m==2)
   ...
   {  
      LCD_putchar(c_p + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_p + 0x30);
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   } 
   printf(LCD_putchar,":"); 
   if(m==3)
   ...
   {
      LCD_putchar(c_g + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_g + 0x30);
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   }
   printf(LCD_putchar,"    ");
   ...
   {
      LCD_putchar(c_d + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_d + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   }
   printf(LCD_putchar,"/"); 
   if(m==5)
   ...
   {
      LCD_putchar(c_t + 0x30);  
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_t + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   }
   printf(LCD_putchar,"/");  
   ...
   {
      LCD_putchar(c_n + 0x30); 
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
      LCD_putchar(dv_n + 0x30);
      // doi ma thap phan sang ASCII va xuat hien thi len LCD
   }                         
   printf(LCD_putchar,"    ");
}
...
void main()         
{
   ...
   printf(LCD_putchar,"     LCD CLOCK!   ");  // xuat hien thi chuoi len LCD 
   LCD_putcmd(0xC0); // lenh dua co tro  den dong 2 cot 1 
   printf(LCD_putchar,"    DUY DH "); // xuat hien thi chuoi len LCD  
   LCD_putcmd(0x94);
   DELAY_MS(1000);
   ...
   while(true)
   {
      ...
      if(nho_giay!=dv_g) 
      // neu giay nho và don vi giay moi doc khac nhau. tuc giay da tang len
      {   
         nho_giay = dv_g; // nho lai giay da thay doi
         if(h1==(c_h*10+dv_h)&&p1==(c_p*10+dv_p)&&g1==(c_g*10+dv_g))
         // dat den thoi gian bao thuc dat
         {
            output_high(coi); 
            // bat coi khi thoi gian thuc = thoi gian bao thuc dat
            dang_bao_thuc=1;
            // bao da bat coi bao thuc
         }
      }  
   ...
   while(true)
    {
         ...
         // khi bien lap i trong lan quet 5-9 thi hien thi dau '?'
         //cho bien minh dang chon gia tri chinh nao
            ...
            switch(mode) 
            {
               case 1:          
                  if(h1>0) 
                     ...
                     h1 = 23; 
                //gio lon hon 0 thi giam 1 don vi con be hon 0 thi nhay ve 23
                  break;
               case 2:        
                  if(p1>0) 
                     p1--;
                     ...
                     p1 = 59;
               //phut lon hon 0 thi giam 1 don vi con be hon 0 thi nhay ve 59
                  break;
               case 3: 
                  if(g1>0) 
                     g1--;
                     ...
                     g1 = 59;
                //giay lon hon 0 thi giam 1 don vi con be hon 0 thi nhay ve 59
                  break;
            }
         }
   ...
   while(true) // vong lap vo han. chi thoat neu co lenh break cua ham
    {
     ...
     //cho bien minh dang chon gia tri chinh nao
         ...
         {  
            ...
            switch(mode) // lua chon tang gia tri tuy thuoc vao bien mode      
            {
               case 1: // mode = 1 // tang gio gio 
                  t = c_h*10+dv_h;
                  // ghep 2 so thap phan rieng le vao mot bien tam
         ...
         {  
            ...
            switch(mode)
            {
               ...
               case 2: 
                  ...
                  c_p = t/10; 
                  dv_p = t%10;
                  //chuc phuc + dvi phut gio nho hon 0 thi nhay ve 59
                  break;
               case 3: 
                  t = c_g*10+dv_g; 
                  if(t>0) 
                  ...
                  c_g = t/10; 
                  dv_g = t%10;
                  //chuc giay + dvi giay gio nho hon 0 thi nhay ve 59
                  break;
               case 4: 
                  t = c_d*10+dv_d; 
                  if(t>1) 
               ...
               case 5: 
                  ...
                  c_t = t/10; 
                  dv_t = t%10;
                  //chuc thang + dvi thango nho hon 1 thi nhay ve 12
                  break;
               case 6: 
                  t = c_n*10+dv_n; 
                  if(t>0) 
         ...
         { 
            ...
            if(mode>6)  
               break; 
             // khi nhan chinh het 6 gia tri thi thoat khoi ham chinh thoi gian
         }
         if(input(k5)==0) 
         // khi nhan nut chinh bao thuc thi thoat khoi ham chinh thoi gian
         {               
            ...
            break; // thoat khoi ham chinh thoi gian
                                    ...
                                    //======================================== 
   mang_tg_rtc[0] = (c_g<<4) | dv_g;
   // chuyen 2 so thap phan cua giay thanh BCD luu vao mang thoi gian
   mang_tg_rtc[1] = (c_p<<4) | dv_p;
   // chuyen 2 so thap phan cua phut thanh BCD luu vao mang thoi gian
   mang_tg_rtc[2] = (c_h<<4) | dv_h;
   // chuyen 2 so thap phan cua gio thanh BCD luu vao mang thoi gian
   mang_tg_rtc[3] = thu;                     
   mang_tg_rtc[4] = (c_d<<4) | dv_d;
   // chuyen 2 so thap phan cua ngay thanh BCD luu vao mang thoi gian
   mang_tg_rtc[5] = (c_t<<4) | dv_t;
   // chuyen 2 so thap phan cua thang thanh BCD luu vao mang thoi gian
   mang_tg_rtc[6] = (c_n<<4) | dv_n;
   // chuyen 2 so thap phan cua nam thanh BCD luu vao mang thoi gian
...
void ReadRTC(int8 * buff) // doc du lieu tu 1307
{
   ...
   *(buff+0)=i2c_Read(1); 
   // Doc giay voi bit thua nhan (ACK) de dia chi tang tu dong
   ...
   *(buff+6)=i2c_Read(0); 
   // doc nam khong bit thua nhan de ket thuc viec doc
